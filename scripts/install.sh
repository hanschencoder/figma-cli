...
#
# 环境变量：
#   FIGMA_SKILL_DIR   skill 安装目录，默认 ~/.claude/skills
set -euo pipefail
...
if [ "$MODE" = uninstall ]; then
  ...
  if [ -L "$SKILL_TARGET" ]; then
    linked="$(realpath_of "$SKILL_TARGET" || true)"
    if [ "$linked" = "$SKILL_SRC" ]; then
      rm "$SKILL_TARGET"; info "已移除 $SKILL_TARGET"
    else
      warn "$SKILL_TARGET 指向 ${linked}，不是本仓库装的，保留不动"
    fi
  elif [ -e "$SKILL_TARGET" ]; then
    warn "$SKILL_TARGET 是真实目录而非软链，保留不动（请自行确认后删除）"
  else
    info "没有装过，跳过"
  fi
...
if [ -L "$SKILL_TARGET" ]; then
  linked="$(realpath_of "$SKILL_TARGET" || true)"
  if [ "$linked" = "$SKILL_SRC" ]; then
    dim "已经指向本仓库，内容随仓库更新，无需处理"
  elif [ "$FORCE" = 1 ] || [ -z "$linked" ]; then
    rm "$SKILL_TARGET"
    ln -s "$SKILL_SRC" "$SKILL_TARGET"
    warn "原软链指向 ${linked:-已失效的路径}，已替换"
  else
    die "$SKILL_TARGET 指向 ${linked}，不是本仓库。加 --force 接管。"
  fi
elif [ -e "$SKILL_TARGET" ]; then
  # 真实目录：可能是早期手动拷进去的版本，备份后换成软链
  bak="$(backup_path "$SKILL_TARGET")"
  mv "$SKILL_TARGET" "$bak"
  ln -s "$SKILL_SRC" "$SKILL_TARGET"
  warn "原目录已备份到 $bak"
else
...
info "$SKILL_TARGET -> $SKILL_SRC"

# ---------------------------------------------------------------- 记录

mkdir -p "$STATE_DIR"
node -e '
  const [file, version, commit, repo, skill, bin] = process.argv.slice(1);
  require("fs").writeFileSync(file, JSON.stringify(
    { version, commit, repoRoot: repo, skillTarget: skill, binPath: bin,
      installedAt: new Date().toISOString() }, null, 2) + "\n");
' "$RECORD" "$VERSION" "$COMMIT" "$REPO_ROOT" "$SKILL_TARGET" "$BIN_PATH"
